...
{
	return (map_x >= 0 && map_x < data->map->width && 
		map_y >= 0 && map_y < data->map->height);
}

/* Draw a portal circle at the specified location */
static void	draw_portal_circle(t_data *data, t_coord portal_pos, int size, int color)
{
	int		x;
	int		y;
	t_coord	pixel;
...
static void	init_show_map(t_data *data, t_show_map *map)
{
	...
	map->offset_x = (map->start_map_x - floor(map->start_map_x)) * map->cell_size;
	map->offset_y = (map->start_map_y - floor(map->start_map_y)) * map->cell_size;
...
static void	draw_player(t_data *data, t_show_map *map)
{
	...
	for (int y = -player_size; y <= player_size; y++)
	{
		for (int x = -player_size; x <= player_size; x++)
		{
			...
			{
				t_coord pixel = {map->player_pos.x + x, map->player_pos.y + y};
				if (pixel.x >= 0 && pixel.x < WIDTH && 
					pixel.y >= 0 && pixel.y < HEIGHT)
				{
	...
	double perp_x = -t.dir_y;
	double perp_y = t.dir_x;
	
	t.left_x = map->player_pos.x + (int)(t.dir_x * player_size * 0.3 + perp_x * player_size);
	t.left_y = map->player_pos.y + (int)(t.dir_y * player_size * 0.3 + perp_y * player_size);
	
	t.right_x = map->player_pos.x + (int)(t.dir_x * player_size * 0.3 - perp_x * player_size);
	t.right_y = map->player_pos.y + (int)(t.dir_y * player_size * 0.3 - perp_y * player_size);
	
	// Draw the triangle
	draw_line(data, (t_coord){t.tip_x, t.tip_y}, 
		(t_coord){t.left_x, t.left_y}, 0xFF);
	draw_line(data, (t_coord){t.tip_x, t.tip_y}, 
		(t_coord){t.right_x, t.right_y}, 0xFF);
}
...
static void	draw_portals(t_data *data, t_show_map *map)
{
	...
	if (data->portal1.x != -1)
	{
		...
		display_x = (data->portal1.x - map->start_map_x) * map->cell_size - map->offset_x;
		display_y = (data->portal1.y - map->start_map_y) * map->cell_size - map->offset_y;
		
		// Check if portal is in view area
		if (display_x + map->cell_size >= 0 && display_x < map->cells_x * map->cell_size &&
			display_y + map->cell_size >= 0 && display_y < map->cells_y * map->cell_size)
		{
	...
	{
		// Get the exact position in the display coordinates
		display_x = (data->portal2.x - map->start_map_x) * map->cell_size - map->offset_x;
		display_y = (data->portal2.y - map->start_map_y) * map->cell_size - map->offset_y;
