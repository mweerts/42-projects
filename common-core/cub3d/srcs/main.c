...
/*   Updated: 2025/03/28 16:08:10 by maxweert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	if (!init_portal(&data))
		return (clean_up(&data), EXIT_FAILURE);
	data.minimap.s_img.img = mlx_new_image(data.s_mlx.mlx, WIDTH / 8, WIDTH / 8);
	data.minimap.s_img.addr = mlx_get_data_addr(data.minimap.s_img.img, &data.minimap.s_img.bits_per_pixel, &data.minimap.s_img.line_length, &data.minimap.s_img.endian);
