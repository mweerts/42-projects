...
inline StatusCode getStatusCode(int code)
{
    static const struct {
        int code;
        StatusCode status;
    ...
    static const size_t mapSize = sizeof(statusMap) / sizeof(statusMap[0]);
    
    for (size_t i = 0; i < mapSize; ++i) {
        if (statusMap[i].code == code) {
            return statusMap[i].status;
        }
    }
