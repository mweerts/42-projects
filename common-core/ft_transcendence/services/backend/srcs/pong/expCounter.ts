...
function tanh(x: number): number {
  // handle very large values - tanh approaches 1 as x approaches infinity
  if (x > 20) return 1;
  if (x < -20) return -1;
...
function xpFactor(xpA: number, xpB: number): number {
  if (isNaN(xpA) || isNaN(xpB) || !isFinite(xpA) || !isFinite(xpB)) {
    console.error(`Invalid XP values in xpFactor: xpA=${xpA}, xpB=${xpB}`);
    return 1.0; // Default factor if inputs are invalid
  }
